...
def _from_ordinalf(x, tz=None, naive=False):
    """
    ...
    if naive and tz is not None:
        raise ValueError('Cannot specify both a value for `tz`'
                         +' and `naive=True` at the same time')
    if tz is None:
        tz = _get_rc_timezone()
...
def num2date(x, tz=None, naive=False):
    """
    ...
    For details, see the module docstring.
    """
    if tz is None and not naive:
        tz = _get_rc_timezone()
    return _from_ordinalf_np_vectorized(x, tz, naive).tolist()
